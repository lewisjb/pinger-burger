...
# Checks dependencies
check_deps() {
  # Check they have netcat
  if [ ! "`command -v netcat`" ]; then
    echo "Script requires GNU 'netcat'" >&2
    exit 1
  fi
  ...
  # Check they have GNU netcat
  if [ -z "`netcat --help | grep GNU`" ]; then
    echo "You have 'netcat', but it isn't GNU 'netcat'" >&2
    exit 1
  fi
...
setup() {
  ...
  mkfifo "$FIFO"
  if [ ! $? ]; then
    echo "Script unable to make fifo $FIFO" >&2
    exit 1
  fi
...
while true ; do
  # Pass the output into the server
  cat "$FIFO" | netcat -k -l -p $PORT | (
